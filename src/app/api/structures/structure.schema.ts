...
const adresseTypologieSchema = z.object({
  ...
  date: z.date({ message: "La date de la typologie d'adresse est requise" }),
...
const contactSchema = z.object({
  ...
  email: z.string().min(1, "L'email du contact est requis"),
  role: z.string().min(1, "Le rôle du contact est requis"),
});

const structureTypologieSchema = z.object({
  date: z.date({ message: "La date de la typologie est requise" }),
...
export const structureCreationSchema = z.object({
  ...
  debutConvention: z.date().optional(),
  finConvention: z.date().optional(),
  cpom: z.boolean({
    required_error: "Le CPOM est requis",
  }),
  creationDate: z.date({ message: "La date de création est requise" }),
  ...
  debutPeriodeAutorisation: z.date().optional(),
  finPeriodeAutorisation: z.date().optional(),
  debutCpom: z.date().optional(),
  finCpom: z.date().optional(),
