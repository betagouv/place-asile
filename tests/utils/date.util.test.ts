...
describe("date util", () => {
  ...
  describe("getMonthsBetween", () => {
    ...
    it("should return a single month if start and end dates are in the same month", () => {
      ...
      // THEN
      expect(result).toEqual(["FÉVR. 2025"]);
    });
    ...
    it("should return the correct months between two dates", () => {
      ...
      expect(result2).toEqual(["JANV. 2025", "FÉVR. 2025"]);
      expect(result3).toEqual(["MARS 2025", "AVR. 2025", "MAI 2025"]);
    });
    ...
    it("should handle Date objects as input", () => {
      ...
      // THEN
      expect(result).toEqual(["JANV. 2025", "FÉVR. 2025", "MARS 2025"]);
    });
  });
  ...
  describe("getLastMonths", () => {
    ...
    it("should return the last N months including the current month", () => {
      ...
      const now = dayjs("2025-04-10");
      const mockDayjs = vi.fn(() => now);
      vi.stubGlobal("dayjs", mockDayjs);
      ...
      // THEN
      expect(result1).toEqual(["AVR. 2025"]);
      expect(result3).toEqual(["FÉVR. 2025", "MARS 2025", "AVR. 2025"]);
