...
export const createFakeCpoms = async (
    ...
    for (const operateur of operateurs) {
        for (const structure of operateur.structures) {
            const region = departementToRegion.get(structure.departementAdministratif);
            if (!region) {
                continue;
            }
            const key = `${operateur.id}-${region}`;
            const existingStructures = structuresByOperateurAndRegion.get(key) || [];
            structuresByOperateurAndRegion.set(key, [...existingStructures, structure.id]);
        }
    }

    // Filter groups with at least n structures and randomize selection
    const validGroups = faker.helpers.shuffle(
        Array.from(structuresByOperateurAndRegion.entries())
            .filter(([, structures]) => structures.length >= minStructuresPerCpom))
        .slice(0, maxCpoms);

    if (validGroups.length === 0) {
        console.log(`⚠️ Aucun groupe (opérateur + région) avec au moins ${minStructuresPerCpom} structures trouvé`);
        return;
    }
    ...
    for (const [key, structures] of validGroups) {
        ...
        const anneeDebut = faker.number.int({ min: currentYear - dureeAnnees, max: currentYear });
        ...
        const nbStructures = Math.max(
            ...
            Math.floor(
                structures.length * faker.number.float({ min: 0.6, max: 1.0 })
            )
        );
                ...
                structures: {
                    create: selectedStructures.map((structureId) => {
                        ...
                        if (joinLater) {
                            ...
                            const joinMax = Math.min(anneeDebut + dureeAnnees - 1, currentYear);
                            if (joinMin <= joinMax) {
                                const anneeRejointe = faker.number.int({ min: joinMin, max: joinMax });
                                dateDebut = new Date(anneeRejointe, 0, 1, 13);
                            }
                        }

                        if (leaveEarly && !joinLater) {
                            const leaveMax = Math.min(anneeDebut + dureeAnnees - 1, currentYear - 1);
                            const leaveMin = Math.min(Math.max(anneeDebut + 1, anneeDebut), leaveMax);
                            if (leaveMin <= leaveMax) {
                                const anneeQuitte = faker.number.int({ min: leaveMin, max: leaveMax });
        ...
        const annees = [...Array(dureeAnnees)].map((_, i) => anneeDebut + i);
        ...
        for (const annee of annees) {
            ...
            await prisma.cpomTypologie.create({
                data: {
                    cpomId: cpom.id,
                    date: typologieDate,
                    cumulResultatNet: faker.number.float({ min: -100000, max: 500000, fractionDigits: 2 }),
                    repriseEtat: faker.number.float({ min: 0, max: 50000, fractionDigits: 2 }),
                    affectationTotal: faker.number.float({ min: 0, max: 1000000, fractionDigits: 2 }),
                    affectationReserveInvestissement: faker.number.float({ min: 0, max: 200000, fractionDigits: 2 }),
                    affectationChargesNonReproductibles: faker.number.float({ min: 0, max: 100000, fractionDigits: 2 }),
                    affectationReserveCompensationDeficits: faker.number.float({ min: 0, max: 150000, fractionDigits: 2 }),
                    affectationReserveCouvertureBFR: faker.number.float({ min: 0, max: 100000, fractionDigits: 2 }),
                    affectationReserveCompensationAmortissements: faker.number.float({ min: 0, max: 200000, fractionDigits: 2 }),
                    affectationFondsDedies: faker.number.float({ min: 0, max: 300000, fractionDigits: 2 }),
                    affectationReportANouveau: faker.number.float({ min: 0, max: 50000, fractionDigits: 2 }),
                    affectationAutre: faker.number.float({ min: 0, max: 100000, fractionDigits: 2 }),
                    commentaire: faker.datatype.boolean({ probability: 0.3 }) ? faker.lorem.sentence() : null,
                },
            });
        }
    }
